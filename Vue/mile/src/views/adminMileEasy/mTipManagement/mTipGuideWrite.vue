...
export default {
  ...
  methods: {
    ...
    async submitForm() {
      ...
      formData.append('user_no', this.getLoginInfo.user_no);
      formData.append('user_name', this.getLoginInfo.user_name);
      if (this.form.file) {
      ...
      try {
        const response = await api.post('/notice/mTipGuideWrite', formData, {
          headers: {
            'Content-Type': 'multipart/form-data',
          },
        });
        ...
        if (response.status === 200) {
          this.showAlert('M-tIP 가이드가 등록되었습니다.', 'success');
          this.$router.push('/mTipMainAdminView');
        } else {
          this.showAlert('M-tIP 가이드 등록 중 오류가 발생했습니다.', 'error');
        }
      } catch (error) {
